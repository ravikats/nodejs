...
Every run checks active policies. If today's date equals `target_date - reminder_days`, it sends an email and records the send in `sent_reminders` so the same reminder is not sent twice.
